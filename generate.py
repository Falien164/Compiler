class LLVMGenerator:
    ...
    def printf_i32(self, id):
        ...
        self.main_text += f"%{self.reg} = call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([4 x i8], [4 x i8]*  @strpi, i32 0, i32 0), i32 %{self.reg-1})\n"
        self.reg += 1

    @staticmethod
    def printf_undefined_i32(self, id):
        self.main_text += f"%{self.reg} = call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([4 x i8], [4 x i8]*  @strpi, i32 0, i32 0), i32 {id})\n"
        self.reg += 1

    @staticmethod
    def printf_real(self, id):
        self.load_real(self, id)
        self.main_text += f"%{self.reg} = call i32 (i8*, ...) @printf(i8* getelementptr inbounds ([4 x i8], [4 x i8]*  @strpd, i32 0, i32 0), double %{self.reg - 1}\n"
        self.reg += 1
    ...
    def sub_i32(self, val1, val2):
        self.main_text += f"%{self.reg} = sub i32 {val2}, {val1}\n"
        self.reg += 1

    @staticmethod
    def sub_real(self, val1, val2):
        self.main_text += f"%{self.reg} = fsub double {val2}, {val1}\n"
        self.reg += 1
